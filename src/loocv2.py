...
def evaluate_metrics_loocv2(
    ...
    for u in users_to_evaluate:
        ...
        all_item_scores = predict_scores2(
        ...
        test_item_pos_array = np.where(unrated_indices == test_item)[0]
        
        if len(test_item_pos_array) == 0:
            # Test item non tra i candidati (errore nei dati)
            continue
            
        test_item_pos_in_candidates = test_item_pos_array[0]
